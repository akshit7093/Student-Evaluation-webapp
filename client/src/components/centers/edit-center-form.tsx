...
// Create a schema for the center form
...
export default function EditCenterForm({ centerId, onSuccess }: EditCenterFormProps) {
  ...
  const { data: center, isLoading, error } = useQuery({
    ...
    queryFn: () => api.getCenterById(centerId),
  });
  ...
  const form = useForm<CenterFormValues>({
    ...
    defaultValues: {
      ...
      address: "",
      city: "Delhi",
      location: { lat: 28.65, lng: 77.22 },
      managerId: null,
      active: true,
    },
  });

  // Update form values when center data is loaded
  useEffect(() => {
    if (center) {
      form.reset({
        name: center.name,
        ...
        city: center.city,
        location: center.location as { lat: number, lng: number },
  ...
  const updateCenter = useMutation({
    mutationFn: async (data: CenterFormValues) => {
      const response = await apiRequest("PATCH", `/api/centers/${centerId}`, data);
      if (!response.ok) {
    ...
    onSuccess: () => {
      ...
      toast({
        ...
        description: "The educational center has been updated.",
      });
      ...
      if (onSuccess) {
        ...
        // Otherwise navigate back to center detail
  ...
  if (error) {
    return (
      <div className="bg-red-500/10 text-red-500 p-4 rounded-md mb-6">
        An error occurred while loading the center information. Please try again.
      </div>
    );
  }
